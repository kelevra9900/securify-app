...
import {mapPastStatus,type RoundListItem} from '@/types/rounds';
...
const RoundsScreen = () => {
	...
	const {data: rounds,isError,isLoading,isRefetching,refetch} = useAvailableRounds();

	const hero = useMemo<null | RoundListItem>(() => {
		if (!rounds?.length) {return null;}
		return rounds.find((r) => r.status === 'IN_PROGRESS')
	...
	if (isLoading) {
		return (
			<CSafeAreaView edges={['top']} style={[styles.centerBase,{backgroundColor: theme.background}]}>
	...
	if (isError) {
		return (
			<CSafeAreaView edges={['top']} style={[styles.centerBase,{backgroundColor: theme.background}]}>
				<Text style={[styles.errBase,{color: theme.textPrimary}]}>No pudimos cargar tus rondas.</Text>
