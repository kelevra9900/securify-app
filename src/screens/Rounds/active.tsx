...
export default function RoundActiveScreen() {
	...
	return (
		<CSafeAreaView edges={['top']} style={{backgroundColor: darkTheme.background,flex: 1}}>
			<Header
				rightSlot={
					<Text onPress={onEnd} style={{color: darkTheme.error,fontWeight: '600'}}>
						Terminar
					</Text>
				}
			...
			<View style={styles.headerBox}>
				<Text style={styles.progressText}>{completed}/{total} completados</Text>
				<Text style={styles.meta}>{isFetching ? 'Actualizando…' : data.remainingMinutes != null ? `Restan ${data.remainingMinutes} min` : '—'}</Text>
				{nextCp && (
					<TouchableOpacity onPress={() => onLog(nextCp.id)} style={styles.primaryBtn}>
						<Text style={styles.primaryBtnText}>Registrar siguiente: {nextCp.location}</Text>
					</TouchableOpacity>
				)}
			</View>
				...
				renderItem={({item}) => {
					...
					return (
						<View style={[styles.cpRow,done && styles.cpDone]}>
							...
							<Text style={styles.cpMeta}>{done ? 'Registrado' : 'Pendiente'}</Text>
							{!done && (
								<TouchableOpacity onPress={() => onLog(item.id)} style={styles.smallBtn}>
...
const styles = StyleSheet.create({
	...
	cpRow: {
		...
		borderColor: darkTheme.border,borderRadius: 10,
		borderWidth: StyleSheet.hairlineWidth,gap: 6,padding: 12,
	},
	headerBox: {gap: 8,padding: 16},
	meta: {color: darkTheme.textSecondary},
	primaryBtn: {alignItems: 'center',backgroundColor: darkTheme.highlight,borderRadius: 10,marginTop: 8,padding: 12},
	primaryBtnText: {color: '#fff',fontWeight: '700'},
	progressText: {color: darkTheme.textPrimary,fontSize: 16,fontWeight: '700'},
	smallBtn: {alignSelf: 'flex-start',backgroundColor: darkTheme.highlight,borderRadius: 8,marginTop: 6,paddingHorizontal: 10,paddingVertical: 6},
