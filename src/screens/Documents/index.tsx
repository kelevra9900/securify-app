...
import {getExtFromName,getFileNameFromPath,pickFileIcon} from '@/utils/documents';
...
const MyDocumentsScreen = () => {
	const {
		...
		{includeSignedUrl: true,perPage: 20,sortBy: 'createdAt',sortOrder: 'desc'},
		{refetchOnReconnect: true,refetchOnWindowFocus: false,staleTime: 60_000},
	);

	const items = useMemo<UserDocumentDTO[]>(() => data?.pages.flatMap((p) => p.data) ?? [],[data]);
		...
		try {
			const url = doc.signedUrl
				...
				: (await getDocumentSignedUrl(doc.id,{
					downloadName: doc.documentType?.slug ? `${doc.documentType.slug}-${doc.id}` : undefined,
				})).url;
	...
	const renderItem: ListRenderItem<UserDocumentDTO> = ({item}) => {
		...
		const badgeBox = expired ? styles.badgeDanger : expSoon ? styles.badgeWarn : styles.badgeOk;
		const badgeText = expired ? styles.badgeTextLight : expSoon ? styles.badgeTextDark : styles.badgeTextLight;

		const suggestedName =
			item.documentType?.slug ? `${item.documentType.slug.toUpperCase()}_${item.id}` : getFileNameFromPath(item.filePath);
		const ext = getExtFromName(getFileNameFromPath(item.filePath));

		const Icon = pickFileIcon(ext);

		return (
			<TouchableOpacity activeOpacity={0.85} onPress={() => openDoc(item)} style={styles.card}>
				...
				<View style={styles.bottomRow}>
					<View style={styles.fileInfo}>
						...
						<Text numberOfLines={1} style={styles.fileName}>{suggestedName}</Text>
						{!!ext && (
	...
	if (items.length === 0) {
		return (
			<CSafeAreaView edges={['top']} style={styles.centerScreen}>
				...
				<Text style={styles.emptySubtitle}>Sube tu primer documento desde la sección correspondiente.</Text>
				<Text onPress={() => refetch()} style={styles.retryLink}>
					Actualizar
				</Text>
			</CSafeAreaView>
	...
	return (
		<CSafeAreaView edges={['top']} style={{backgroundColor: darkTheme.background,flex: 1}}>
			<Header
				rightSlot={<Text onPress={() => refetch()} style={{color: darkTheme.highlight}}>Actualizar</Text>}
...
const styles = StyleSheet.create({
	...
	title: {color: darkTheme.textPrimary,flex: 1,fontSize: 16,fontWeight: '600',marginRight: 8},
	...
	bottomRow: {alignItems: 'center',flexDirection: 'row',justifyContent: 'space-between'},
	extPill: {
	...
	extPillText: {color: darkTheme.textSecondary,fontSize: 10,fontWeight: '600'},
	fileInfo: {alignItems: 'center',flex: 1,flexDirection: 'row',gap: 8},
	fileName: {color: darkTheme.textSecondary,flex: 1,fontSize: 12,marginLeft: 6},
	// States
	centerScreen: {
		alignItems: 'center',
		backgroundColor: darkTheme.background,
	...
	emptySubtitle: {color: darkTheme.textSecondary,fontSize: 13,marginBottom: 8,textAlign: 'center'},
	emptyTitle: {color: darkTheme.textPrimary,fontSize: 18,fontWeight: '600',marginBottom: 6,textAlign: 'center'},
