	...
	return (
		...
		>
			<View style={[styles.modal,{backgroundColor: theme.cardBackground}]}>
				<Text style={[styles.title,{color: theme.textPrimary}]}>Detalle del Checkpoint</Text>

				<View style={styles.row}>
					<MapPin color={theme.textSecondary} size={20} />
					<Text style={[styles.text,{color: theme.textPrimary}]}>{checkpointName}</Text>
				</View>

				{guardName && (
					<View style={styles.row}>
						<User color={theme.textSecondary} size={20} />
						<Text style={[styles.text,{color: theme.textPrimary}]}>{guardName}</Text>
					</View>
				)}
				...
				<View style={styles.row}>
					...
					<Text style={[styles.text,{color: theme.textPrimary}]}>
						{timestamp ? new Date(timestamp).toLocaleTimeString() : 'Sin hora registrada'}
					</Text>
				</View>

				<View style={styles.row}>
					<View style={[styles.statusIndicator,{backgroundColor: statusDetails[status].color}]} />
					<Text style={[styles.text,{color: theme.textPrimary}]}>{statusDetails[status].label}</Text>
				</View>
